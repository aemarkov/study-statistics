...
#region HISTOGRAM
		...
		public DisplayForm(Dictionary<Range, double> data,string xName, string valName)
		{
			...
			var pane = graph.GraphPane;
			double maxY = 0;
			double interval = data.First().Key.Right - data.First().Key.Left;
			...
			{
				var box = new BoxItem(new RectangleF((float)xi[i], (float)height[i], (float)interval, (float)height[i]), Color.Black, Color.FromArgb(255, 39, 174, 96));
